...
local function leap(kwargs)
  ...
  local function _34_(_, k)
    ...
    if (nil ~= _35_) then
      ...
      return map(replace_keycodes, _36_())
    else
      return nil
    end
  end
  ...
  local function post_pattern_input_loop(targets)
    ...
    local function display()
      ...
      return with_highlight_chores(_80_)
    end
  ...
  local function traversal_loop(targets, start_idx, _88_)
    local use_no_labels_3f = _88_["use-no-labels?"]
    ...
    local function loop(idx, first_invoc_3f)
      ...
      if (nil ~= _92_) then
        local _in = _92_
        local _93_ = traversal_get_new_idx(idx, _in, targets)
        if (nil ~= _93_) then
          local new_idx = _93_
  ...
  if invoked_dot_repeat_3f then
    local _103_ = targets[state.dot_repeat.target_idx]
    if (nil ~= _103_) then
      local target = _103_
      do_action(target)
      exit_2a()
  ...
  local function exit_with_action_on_2a(idx)
    ...
    set_dot_repeat(in1, _3fin20, _120_())
    do_action(target)
    return exit_2a()
  end
  if count then
  ...
  elseif (contains_3f(spec_keys.next_target, in_final) and (st["curr-idx"] == 0)) then
    ...
    local _125_, _126_ = get_target_with_active_label(targets_2a, in_final)
    if ((nil ~= _125_) and (nil ~= _126_)) then
      local target = _125_
      local idx = _126_
...
local function init()
  ...
  local function set_concealed_label()
    ...
    if leap_label.bg then
      ...
      opts.concealed_label = middle_dot
    end
    return nil
  end
  ...
  api.nvim_create_autocmd("ColorScheme", {group = "LeapDefault", callback = _131_})
  ...
  api.nvim_create_autocmd("User", {pattern = "LeapEnter", group = "LeapDefault", callback = _137_})
  local function _138_(_)
    return restore_editor_opts()
  end
  return api.nvim_create_autocmd("User", {pattern = "LeapLeave", group = "LeapDefault", callback = _138_})
end
init()
