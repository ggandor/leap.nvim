...
local function leap(kwargs)
  ...
  local function prepare_pattern(in1, _3fin2)
    ...
    return ("\\V" .. _97_() .. (expand_to_user_defined_character_class(in1) or in1:gsub("\\", "\\\\")) .. (expand_to_user_defined_character_class(_3fin2) or _3fin2 or "\\_."))
  end
  ...
  local function post_pattern_input_loop(sublist, _3fgroup_offset, first_invoc_3f)
    local function loop(group_offset, first_invoc_3f0)
      do
        local _123_ = sublist
        set_label_states(_123_, {["group-offset"] = group_offset})
        set_beacons(_123_, {["user-given-targets?"] = user_given_targets, ["aot?"] = aot_3f})
      end
      do
        hl:cleanup(hl_affected_windows)
        if not (user_given_targets and not _3ftarget_windows) then
        ...
        do
          ...
          light_up_beacons(sublist, _125_())
        end
        hl["highlight-cursor"](hl)
        vim.cmd("redraw")
      end
    ...
    return loop((_3fgroup_offset or 0), _132_())
  end
    ...
    local function loop(targets)
      ...
      elseif ((nil ~= _133_) and (nil ~= _134_)) then
        ...
        do
          ...
          if ((_G.type(_138_) == "table") and (nil ~= (_138_)[1]) and (nil ~= (_138_)[2])) then
            local idx = (_138_)[1]
  ...
  local function traversal_loop(targets, idx, _142_)
    ...
    local force_no_labels_3f = _arg_143_["force-no-labels?"]
    ...
    if (nil ~= _146_) then
      ...
      if ((input == spec_keys.next_match) or (input == spec_keys.prev_match)) then
        ...
        do
          local _148_ = input
          if (_148_ == spec_keys.next_match) then
            new_idx = min(inc(idx), #targets)
          elseif (_148_ == spec_keys.prev_match) then
        ...
        update_state({["repeat"] = {in1 = state["repeat"].in1, in2 = _151_}})
  ...
  local function _159_(...)
    ...
    if ((nil ~= _160_) and true) then
      ...
      local function _162_(...)
        ...
        if (nil ~= _163_) then
                    ...
                    if (#targets_2a == 1) then
                      ...
                      if (nil ~= _176_) then
                        ...
                        if ((in_final == spec_keys.next_match) and directional_3f) then
                          ...
                          local _179_ = get_target_with_active_primary_label(targets_2a, in_final)
                          if ((_G.type(_179_) == "table") and (nil ~= (_179_)[1]) and true) then
                            local idx = (_179_)[1]
                            local _2 = (_179_)[2]
          ...
          local function _199_(...)
            if dot_repeat_3f then
              local _190_ = targets[state.dot_repeat.target_idx]
              if (nil ~= _190_) then
                local target = _190_
                do
                  do_action(target)
              ...
              local function _193_(_241)
                local _194_ = _241
                set_autojump(_194_, force_noautojump_3f)
                attach_label_set(_194_)
                set_labels(_194_, multi_select_3f)
                return _194_
              end
              prepare_targets = _193_
              if _3fin2 then
      ...
      return _162_((user_given_targets or _201_(...) or _202_(...)))
    elseif true then
      local __60_auto = _160_
      return ...
    else
      return nil
    end
...
local function set_editor_opts(opts0)
  for opt, val in pairs(opts0) do
    local _let_207_ = vim.split(opt, ".", true)
    local _ = _let_207_[1]
    local scope = _let_207_[2]
    local name = _let_207_[3]
