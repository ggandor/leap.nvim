  ...
  for i, _19_ in ipairs(match_positions) do
    ...
    if not (skip_curpos_3f and (line == curline) and (col == curcol)) then
      ...
      if (ch1 == "") then
        ...
        skip_match_3f = (triplet_3f and _24_())
...
local function sort_by_distance_from_cursor(targets, cursor_positions, source_winid)
  ...
  local _let_33_ = (cursor_positions[source_winid] or {-1, -1})
  local source_line = _let_33_[1]
  local source_col = _let_33_[2]
...
local function get_targets(pattern, _50_)
  local _arg_51_ = _50_
  local backward_3f = _arg_51_["backward?"]
  local match_same_char_seq_at_end_3f = _arg_51_["match-same-char-seq-at-end?"]
  local target_windows = _arg_51_["target-windows"]
