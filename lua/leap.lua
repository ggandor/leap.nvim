...
local function to_closed_fold_edge_21(reverse_3f)
  ...
  local _60_
  if reverse_3f then
    _60_ = vim.fn.foldclosed
  else
    _60_ = vim.fn.foldclosedend
  end
  edge_line = _60_(vim.fn.line("."))
...
local function get_match_positions(pattern, _71_, _73_)
  local _arg_72_ = _71_
  local left_bound = _arg_72_[1]
  local right_bound = _arg_72_[2]
  local _arg_74_ = _73_
  local reverse_3f = _arg_74_["reverse?"]
  local whole_window_3f = _arg_74_["whole-window?"]
  local skip_curpos_3f = _arg_74_["skip-curpos?"]
  local skip_orig_curpos_3f = skip_curpos_3f
  local _let_75_ = get_cursor_pos()
  local orig_curline = _let_75_[1]
  local orig_curcol = _let_75_[2]
  ...
  local function iter(match_at_curpos_3f)
    ...
    local _81_ = vim.fn.searchpos(pattern, flags, stopline)
    if ((_G.type(_81_) == "table") and (nil ~= (_81_)[1]) and (nil ~= (_81_)[2])) then
      ...
      elseif ((col < left_bound) and (col > right_bound) and not vim.wo.wrap) then
        local _84_ = to_next_in_window_pos_21(reverse_3f, left_bound, right_bound, stopline)
      ...
      elseif (vim.fn.foldclosed(line) ~= -1) then
        ...
        local _86_ = skip_one_21(reverse_3f)
        if (_86_ == "dead-end") then
          return cleanup()
        elseif true then
          local _ = _86_
          return iter(true)
...
local function get_targets_2a(pattern, _90_)
  local _arg_91_ = _90_
  local reverse_3f = _arg_91_["reverse?"]
  local wininfo = _arg_91_["wininfo"]
  local targets = _arg_91_["targets"]
  local source_winid = _arg_91_["source-winid"]
  local targets0 = (targets or {})
  local _let_92_ = get_horizontal_bounds()
  local _ = _let_92_[1]
  local right_bound = _let_92_[2]
  local bounds = _let_92_
  ...
  local prev_match = {}
  for _93_ in match_positions do
    ...
    do
      local _95_ = char_at_pos(pos, {["char-offset"] = 1})
      if (nil ~= _95_) then
        local char = _95_
        ch2, eol_3f = char
      elseif true then
    ...
    same_char_triplet_3f = ((ch2 == prev_match.ch2) and (line == prev_match.line) and (col == _97_(prev_match.col)))
...
local function distance(_101_, _103_)
  local _arg_102_ = _101_
  local l1 = _arg_102_[1]
  local c1 = _arg_102_[2]
  local _arg_104_ = _103_
  local l2 = _arg_104_[1]
  local c2 = _arg_104_[2]
  local editor_grid_aspect_ratio = 0.3
  local _let_105_ = {abs((c1 - c2)), abs((l1 - l2))}
  local dx = _let_105_[1]
  local dy = _let_105_[2]
  local dx0 = (dx * editor_grid_aspect_ratio)
  return pow((pow(dx0, 2) + pow(dy, 2)), 0.5)
end
local function get_targets(pattern, _106_)
  local _arg_107_ = _106_
  local reverse_3f = _arg_107_["reverse?"]
  local target_windows = _arg_107_["target-windows"]
  if not target_windows then
    ...
    do
      local _108_ = target_windows
      if ((_G.type(_108_) == "table") and ((_G.type((_108_)[1]) == "table") and (((_108_)[1]).winid == source_winid)) and ((_108_)[2] == nil)) then
    ...
    for _, _110_ in ipairs(target_windows) do
      local _each_111_ = _110_
      local winid = _each_111_["winid"]
      local wininfo = _each_111_
    ...
    if not empty_3f(targets) then
      ...
      if by_screen_pos_3f then
        for winid, _114_ in pairs(cursor_positions) do
          local _each_115_ = _114_
          local line = _each_115_[1]
          local col = _each_115_[2]
          local _116_ = vim.fn.screenpos(winid, line, col)
          if ((_G.type(_116_) == "table") and ((_116_).col == col) and (nil ~= (_116_).row)) then
            local row = (_116_).row
      ...
      for _, _119_ in ipairs(targets) do
        local _each_120_ = _119_
        local _each_121_ = _each_120_["pos"]
        local line = _each_121_[1]
        local col = _each_121_[2]
        local _each_122_ = _each_120_["wininfo"]
        local winid = _each_122_["winid"]
        local t = _each_120_
        if by_screen_pos_3f then
...
local function populate_sublists(targets)
  ...
  if opts.case_insensitive then
    local function _129_(t, k)
      return rawget(t, k:lower())
    end
    local function _130_(t, k, v)
      return rawset(t, k:lower(), v)
    end
    setmetatable(targets.sublists, {__index = _129_, __newindex = _130_})
  else
  end
  for _, _132_ in ipairs(targets) do
    local _each_133_ = _132_
    local _each_134_ = _each_133_["pair"]
    local _0 = _each_134_[1]
    local ch2 = _each_134_[2]
    local target = _each_133_
...
local function attach_label_set(sublist)
  ...
  sublist["label-set"] = _136_
  return nil
end
local function set_sublist_attributes(targets, _138_)
  local _arg_139_ = _138_
  local force_no_autojump_3f = _arg_139_["force-no-autojump?"]
...
local function set_labels(targets)
  for _, sublist in pairs(targets.sublists) do
    if (#sublist > 1) then
      local _local_140_ = sublist
      local autojump_3f = _local_140_["autojump?"]
      local label_set = _local_140_["label-set"]
      for i, target in ipairs(sublist) do
        ...
        if (i_2a > 0) then
          local _143_
          do
            local _142_ = (i_2a % #label_set)
            if (_142_ == 0) then
              _143_ = label_set[#label_set]
...
local function set_label_states(sublist, _149_)
  local _arg_150_ = _149_
  local group_offset = _arg_150_["group-offset"]
  ...
  for i, target in ipairs(sublist) do
    if target.label then
      local _152_
      if (function(_153_,_154_,_155_) return (_153_ <= _154_) and (_154_ <= _155_) end)(primary_start,i,primary_end) then
        _152_ = "active-primary"
      elseif (function(_156_,_157_,_158_) return (_156_ <= _157_) and (_157_ <= _158_) end)(secondary_start,i,secondary_end) then
        _152_ = "active-secondary"
      elseif (i > secondary_end) then
        _152_ = "inactive"
      else
        _152_ = nil
      end
      target["label-state"] = _152_
    else
    end
  end
  return nil
...
local function set_beacon_for_labeled(target)
  if target.label then
    local _let_161_ = target
    local _let_162_ = _let_161_["pair"]
    local ch1 = _let_162_[1]
    local ch2 = _let_162_[2]
    local edge_pos_3f = _let_161_["edge-pos?"]
    local label = _let_161_["label"]
    local offset
    ...
    offset = (ch1:len() + _163_())
    local virttext
    do
      local _164_ = target["label-state"]
      if (_164_ == "active-primary") then
        virttext = {{label, hl.group["label-primary"]}}
      elseif (_164_ == "active-secondary") then
        virttext = {{label, hl.group["label-secondary"]}}
      elseif (_164_ == "inactive") then
...
local function resolve_conflicts(target_list)
  ...
  for i, target in ipairs(target_list) do
    ...
    local _let_172_ = _let_171_["pos"]
    local lnum = _let_172_[1]
    local col = _let_172_[2]
    local _let_173_ = _let_171_["pair"]
    local ch1 = _let_173_[1]
    local _ = _let_173_[2]
    local _let_174_ = _let_171_["wininfo"]
    local bufnr = _let_174_["bufnr"]
    local winid = _let_174_["winid"]
    local _175_ = target.beacon
    if (_175_ == nil) then
    ...
    elseif ((_G.type(_175_) == "table") and (nil ~= (_175_)[1]) and true) then
      local offset = (_175_)[1]
      local _0 = (_175_)[2]
      local k = (bufnr .. " " .. winid .. " " .. lnum .. " " .. (col + offset))
      do
        local _178_ = unlabeled_match_positions[k]
        if (nil ~= _178_) then
          local other = _178_
          target.beacon = nil
...
local function light_up_beacons(target_list, _3fstart)
  for i = (_3fstart or 1), #target_list do
    ...
    local _186_ = target.beacon
    if ((_G.type(_186_) == "table") and (nil ~= (_186_)[1]) and (nil ~= (_186_)[2])) then
      local offset = (_186_)[1]
      local virttext = (_186_)[2]
...
local function leap(_189_)
  local _arg_190_ = _189_
  local dot_repeat_3f = _arg_190_["dot-repeat?"]
  local target_windows = _arg_190_["target-windows"]
  local kwargs = _arg_190_
  ...
  local _let_191_ = _192_()
  local reverse_3f = _let_191_["reverse?"]
  local inclusive_op_3f = _let_191_["inclusive-op?"]
  local offset = _let_191_["offset"]
  local _3ftarget_windows
  do
    local _193_ = target_windows
    if (_G.type(_193_) == "table") then
      local t = _193_
      _3ftarget_windows = t
    elseif (_193_ == true) then
  ...
  local spec_keys
  local function _195_(_, k)
    return replace_keycodes(opts.special_keys[k])
  end
  spec_keys = setmetatable({}, {__index = _195_})
  ...
  local function traverse(targets, idx, _207_)
    ...
    local function _211_()
      ...
      local function _214_()
        if _3ftarget_windows then
          local _213_ = _3ftarget_windows
          table.insert(_213_, source_window)
          return _213_
        else
          return nil
        end
    ...
    if (nil ~= _210_) then
      ...
      if ((input == spec_keys.next_match) or (input == spec_keys.prev_match)) then
        ...
        do
          local _215_ = input
          if (_215_ == spec_keys.next_match) then
            new_idx = min(inc(idx), #targets)
          elseif (_215_ == spec_keys.prev_match) then
        ...
        local _217_ = get_target_with_active_primary_label(targets, input)
        if ((_G.type(_217_) == "table") and true and (nil ~= (_217_)[2])) then
          local _ = (_217_)[1]
          local target = (_217_)[2]
          if dot_repeatable_op_3f then
            set_dot_repeat()
          else
          end
  ...
  local function get_first_pattern_input()
    ...
    if (_227_ == spec_keys.repeat_search) then
      if state["repeat"].in1 then
        ...
        local function _234_()
          if _3ftarget_windows then
            local _233_ = _3ftarget_windows
            table.insert(_233_, source_window)
            return _233_
          else
            return nil
          end
  ...
  local function get_second_pattern_input(targets)
    do
      local _237_ = targets
      set_initial_label_states(_237_)
      set_beacons(_237_, {})
    end
    do
      hl:cleanup(_3ftarget_windows)
    ...
    local function _238_()
      ...
      local function _241_()
        if _3ftarget_windows then
          local _240_ = _3ftarget_windows
          table.insert(_240_, source_window)
          return _240_
        else
          return nil
        end
    ...
    if ((nil ~= _252_) and true) then
        ...
        if (nil ~= _255_) then
          ...
          local function _256_(...)
            ...
            if (nil ~= _257_) then
              local in2 = _257_
              if dot_repeat_3f then
                local _258_ = targets[state["dot-repeat"]["target-idx"]]
                if (nil ~= _258_) then
                  local target = _258_
                  if dot_repeatable_op_3f then
                    set_dot_repeat()
              ...
              elseif (directional_3f and (in2 == spec_keys.next_match)) then
                ...
                _271_ = (targets.sublists[in2] or _272_(...))
                if ((_G.type(_271_) == "table") and (nil ~= (_271_)[1]) and ((_271_)[2] == nil)) then
                  local only = (_271_)[1]
                  if dot_repeatable_op_3f then
                    set_dot_repeat()
                  else
                ...
                elseif (nil ~= _271_) then
                  ...
                  local _280_ = post_pattern_input_loop(sublist)
                  if (nil ~= _280_) then
                    local in_final = _280_
                    if (directional_3f and (in_final == spec_keys.next_match)) then
                      ...
                      local _286_ = get_target_with_active_primary_label(sublist, in_final)
                      if ((_G.type(_286_) == "table") and (nil ~= (_286_)[1]) and (nil ~= (_286_)[2])) then
                        local idx = (_286_)[1]
          ...
          local function _303_(...)
            do
              local _304_ = targets
              populate_sublists(_304_)
              set_sublist_attributes(_304_, {["force-no-autojump?"] = force_no_autojump_3f})
              set_labels(_304_)
            end
      ...
      local function _306_(...)
        ...
        local function _309_(...)
          if _3ftarget_windows then
            local _308_ = _3ftarget_windows
            table.insert(_308_, source_window)
            return _308_
          else
            return nil
          end
      ...
      return _254_((get_targets(prepare_pattern(in1, _3fin2), {["reverse?"] = reverse_3f, ["target-windows"] = _3ftarget_windows}) or _306_(...)))
    elseif true then
...
api.nvim_create_autocmd("ColorScheme", {callback = _317_, group = "LeapDefault"})
local function _318_()
  save_editor_opts()
  return set_temporary_editor_opts()
end
api.nvim_create_autocmd("User", {pattern = "LeapEnter", callback = _318_, group = "LeapDefault"})
